  ...
  return <DialogPortal open={open} onClose={onClose} className="max-w-2xl w-full">
    ...
    <div className="p-4 space-y-4">
      ...
      <form onSubmit={addOCIPrefix} className="space-y-2">
		...
		<div className="flex gap-2"><Input value={ociPrefix} onChange={event => setOCIPrefix(event.target.value)} placeholder="oci://registry.example.com/charts" disabled={configManaged} /><button type="submit" disabled={configManaged || !ociPrefix.trim().startsWith('oci://') || addOCI.isPending} className="px-3 py-2 rounded-lg bg-accent text-white disabled:opacity-50">Add</button></div>
